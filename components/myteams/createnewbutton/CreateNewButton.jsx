...
import TaskContainer from "./createnewoutside/Taskcontainer/TaskContainer";
import FeedbackContainer from "./createnewoutside/FeedbackContainer/FeedbackContainer";
import CorrectionContainer from "./createnewoutside/CorrectionContainer/CorrectionContainer";
...
function CreateNewButton(props) {
  ...
  const handleCloseFilter = () => {
    setShowFilter(false);
    props.handleShowModalForAll(false);
    props.handleShowBacklogModalForAll(false);
  };
  const handleShowFilter = () => setShowFilter(true);

  useEffect(() => {
    setShowFilter(props.showModalForAll);
  }, [props.showModalForAll]);

  useEffect(() => {
    setShowFilter(props.showBackLogModal);
  }, [props.showBackLogModal]);
    ...
    <div>
      <DropdownButton
        ...
        title={
          <div className="d-flex align-items-center gap-1">
            <FaPlus /> <div className="pdt02">Create New</div>
          </div>
        }
        variant="dark"
        size="sx"
      ...
      <Modal show={showFilter} onHide={handleCloseFilter} size="xl">
        ...
        <Modal.Body className="modal-body-createnew">
          {selectedItem === "Task" ? (
            <TaskContainer selectedItem={selectedItem} handleSubmitJob={handleSubmitJob} />
          ) : selectedItem === "Feedback" ? (
